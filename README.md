...
Author: Techn · Version: 0.3.11 · Branding mode: Author Branded
...
- Each content tab shows WordPress’s runtime Public, Publicly Queryable, Exclude From Search and Hierarchical settings as navy ticks or orange crosses, plus a Native or orange Custom declaration. Publicly Queryable uses a neutral grey dot for native types, with a tooltip and screen-reader explanation that WordPress handles their public visibility. Values refresh with the catalog; unusual non-boolean values are displayed literally for diagnosis.
