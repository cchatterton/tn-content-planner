...
(async()=>{
 ...
 for(const viewport of [{width:1600,height:1050},{width:390,height:844},{width:800,height:700}]){
  ...
  const eyebrowBox=await page.locator('.tncp-eyebrow').boundingBox();
  const tagBox=await page.locator('.tncp-capabilities li').last().boundingBox();
  const watermarkBox=await watermark.boundingBox();
  if(Math.abs(watermarkBox.y-eyebrowBox.y)>1 || Math.abs(watermarkBox.x+watermarkBox.width-tagBox.x-tagBox.width)>1)throw Error('Watermark does not align with eyebrow and final tag');
