...
(async()=>{
 ...
 await checkCounts();
 await row.getByRole('checkbox',{name:'Select Skip this item',exact:true}).check();await page.getByRole('button',{name:'Map Selected',exact:true}).click();
 ...
 const xp=page.getByRole('tab',{name:/^XP Patterns,/});
 assert.match(await page.getByRole('tab').last().innerText(),/^XP Patterns \d+\/\d+$/);await xp.click();
 ...
 const assignee=patternRow.getByRole('combobox',{name:/^XP Owner/});
 ...
 const baseCounts=(await xp.innerText()).match(/(\d+)\/(\d+)$/).slice(1).map(Number);
 assert.equal(baseCounts[1],await page.locator('[data-pattern]').count());
 await patternRow.getByRole('combobox',{name:/^Status/}).selectOption('done');
 assert.equal(await xp.innerText(),`XP Patterns ${baseCounts[0]+1}/${baseCounts[1]}`);
 ...
 assert.equal(await patternRow.getByRole('combobox',{name:/^XP Owner/}).inputValue(),assignedId);
 ...
 const counterBeforeFilter=await xp.innerText();
 ...
 const visibleAssignees=await page.getByRole('combobox',{name:/^XP Owner/}).evaluateAll(nodes=>nodes.map(node=>node.value));
 assert.ok(visibleAssignees.length>0 && visibleAssignees.every(value=>value===assignedId));
 await page.getByRole('button',{name:'Show All',exact:true}).click();
 assert.equal(await xp.innerText(),counterBeforeFilter);
